...
import com.luis.store.MainActivity;
...
public class ConfigPubNub extends SubscribeCallback {
    ...
    public void buildPubSub() {
        ...
        data.put(Utils.PUBSUB_TECHNIQUE,"");
        data.put(Utils.DEVICE_SESSION_ID_KEY,"");
        data.put(Utils.PUBNUB_SUB_KEY,"");
        data.put(Utils.PUBNUB_PUB_KEY,"");
        data.put(Utils.PUBNUB_SEC_KEY,"");
        data=generalFunc.retrieveValue(data);

        if (data.get(Utils.PUBSUB_TECHNIQUE).equalsIgnoreCase("SocketCluster")) {
            ConfigSCConnection.getInstance().buildConnection();
            return;
        } else if (data.get(Utils.PUBSUB_TECHNIQUE).equalsIgnoreCase("PubNub")) {
            PNConfiguration pnConfiguration = new PNConfiguration();
            pnConfiguration.setUuid((data.get(Utils.DEVICE_SESSION_ID_KEY).equals("") ? generalFunc.getMemberId() : data.get(Utils.DEVICE_SESSION_ID_KEY)));

            pnConfiguration.setSubscribeKey(data.get(Utils.PUBNUB_SUB_KEY));
            pnConfiguration.setPublishKey(data.get(Utils.PUBNUB_PUB_KEY));
            pnConfiguration.setSecretKey(data.get(Utils.PUBNUB_SEC_KEY));
...
//        pnConfiguration.setLogVerbosity(PNLogVerbosity.BODY);
    ...
    public void subscribeToPrivateChannel() {
        if (pubnub != null) {
            pubnub.subscribe()
                    .channels(Arrays.asList("COMPANY_" + generalFunc.getMemberId())) // subscribe to channels
                    .execute();
        }
        if (ConfigSCConnection.retrieveInstance() != null) {
            ConfigSCConnection.getInstance().subscribeToChannels("COMPANY_" + generalFunc.getMemberId());
        }
        if (ConfigYalgaarConnection.retrieveInstance() != null) {
            ConfigYalgaarConnection.getInstance().subscribeToChannels("COMPANY_" + generalFunc.getMemberId());
        }
    }
    ...
    public void unSubscribeToPrivateChannel() {
        ...
        if (ConfigSCConnection.retrieveInstance() != null) {
            ConfigSCConnection.getInstance().unSubscribeFromChannels("COMPANY_" + generalFunc.getMemberId());
        }

        if (ConfigYalgaarConnection.retrieveInstance() != null) {
            ConfigYalgaarConnection.getInstance().unSubscribeFromChannels("COMPANY_" + generalFunc.getMemberId());
        }
    }
